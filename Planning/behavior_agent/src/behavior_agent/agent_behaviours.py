...
class Testbev(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        rospy.loginfo(self.blackboard.get("/carla/ego_vehicle/target_speed"))
        return py_trees.common.Status.RUNNING
...
class SubscriberToBlackboard(py_trees.behaviour.Behaviour):
    def __init__(self, name):
        """
        Minimal one-time initialisation. A good rule of thumb is
        to only include the initialisation relevant for being able
        ...
        the setup() method.
        """
        super(SubscriberToBlackboard, self).__init__(name)
