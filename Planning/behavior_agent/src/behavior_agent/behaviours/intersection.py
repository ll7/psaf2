...
class Approach(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        if _tl.data == "red" or _tl.data == "yellow":
            ...
            self.trafficlight_distance = min(_pinfo.relative_y_coord)
        ...
        # if stop line is detected but the distance to the traffic light is too short, we've passed it and need to break immediately
        elif self.stopline_detected and self.trafficlight_distance <= 0.4:
            self.target_speed_pub.publish(0)
            print("STOP TRAFFIC LIGHTS")
...
class Wait(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        self.speed = np.sqrt(
            self.odo.twist.twist.linear.x ** 2 + self.odo.twist.twist.linear.y ** 2 + self.odo.twist.twist.linear.z ** 2) * 3.6
        if self.speed < 5 and _tl.data != "red" and _tl.data != "yellow":
            return py_trees.common.Status.SUCCESS
        else:
            return py_trees.common.Status.RUNNING
