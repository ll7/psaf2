...
class Radar(object):  # pylint: disable=too-few-public-methods
    ...
    def radar_updated(self, msg):
        if self.path != None:
            ...
            poses_transformed = [tf2_geometry_msgs.do_transform_pose(p, trans) for p in poses]
            ...
            for p in poses_transformed:
                ...
                dist = min(d)            
                if dist < 2:
                    dist_indices.append(poses_transformed.index(p))
                    points.append(p)                 
            ...
            z = [p.pose.position.z for p in points]
            xyz = [[p.pose.position.x, p.pose.position.y, p.pose.position.z] for p in points]             
            ...
            # try:
            #     trans = self.tf_buffer.lookup_transform( 'ego_vehicle/radar/front', 'map', rospy.Time())
            # except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
            #     print("Error in transformation")                       

            # poses_transformed_back = [tf2_geometry_msgs.do_transform_pose(p, trans) for p in points]
            
            # dist = [p.pose.position.x for p in poses_transformed_back]
            ...
            if len(dist) > 0:
                ...
                if rospy.get_time() > self.current_time + 2.0:
                    self._dist_publisher.publish(100)   
                    self.current_time = rospy.get_time()            
